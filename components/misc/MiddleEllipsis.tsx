...
const MiddleEllipsis = ({ text }: AppProps): JSX.Element => {
    ...
    if (xs) {
        if (str.length > 10) {
            return (
                <>
                    {str.substr(0, 5)}...{str.substr(str.length - 18, str.length)}
                </>
            );
        } else {
            return <>{str}</>;
        ...
        if (str.length > 12) {
            return (
                <>
                    {str.substr(0, 6)}...{str.substr(str.length - 20, str.length)}
                </>
            );
        } else {
            return <>{str}</>;
        ...
        if (str.length > 15) {
            return (
                <>
                    {str.substr(0, 7)}...{str.substr(str.length - 15, str.length)}
                </>
            );
        } else {
            return <>{str}</>;
